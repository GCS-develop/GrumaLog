...
/**
 * This is the model class for table "Auditoriamanualimportaciondetalle".
 *
 * @property int $id
 ...
 *
 * @property Auditoriamanualimportacion $idInterfase0
 */
class Auditoriamanualimportaciondetalle extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return 'auditoriamanualimportaciondetalle';
    }
    ...
    public function rules()
    {
        return [
            ...
            [['idInterfase'], 'exist', 'skipOnError' => true, 'targetClass' => Auditoriamanualimportacion::class, 'targetAttribute' => ['idInterfase' => 'id']],
        ];
    }
    ...
    {
        return $this->hasOne(Auditoriamanualimportacion::class, ['id' => 'idInterfase']);
    }
    ...
    public static function upload($archivo)
    {
        ...
        if (!$model->save()) {
            ...
            die("hola - Auditoria manual importacion");
        }
        $id = $model->id;

        $respuesta = Auditoriamanualimportaciondetalle::extraer_data_archivo($tempFileName, $model->id);

        $model = Auditoriamanualimportacion::findOne($id);
        $model->numeroRegistros = Auditoriamanualimportaciondetalle::find()->where(['idInterfase' => $id])->count();
        $model->totalCantidad = Auditoriamanualimportaciondetalle::find()->where(['idInterfase' => $id])->sum('cantidad');
        $model->save();
    ...
    public static function extraer_data_archivo($archivoExcel, $id)
    {
        ...
        for ($fila = 2; $fila <= $totalFilas; $fila++) {
            ...
            $model = new Auditoriamanualimportaciondetalle();
            ...
            $modeldocumento = Auditoriamanualdocumento::find()->where(['codigoBodegaSalida' => $model->codigoBodegaSalida, 'numeroDocumento' => $model->numeroDocumento])->one();
            if ($modeldocumento == null) {
                $modeldocumento = new Auditoriamanualdocumento();
            ...
            $detalle = Auditoriamanualdocumentodetalle::find()->where(['idDocumento' => $modeldocumento->id, 'codigoBarras' => $model->codigoBarras])->one();
            if ($detalle == null) {
                $detalle = new Auditoriamanualdocumentodetalle();
