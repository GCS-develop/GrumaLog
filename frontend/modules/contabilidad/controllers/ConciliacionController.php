...
public function actionConciliacion($id = null)
{
    ...
    // ==========================
    // 2) Traer existencias por ITEM + EXT1 + EXT2 + BODEGA (con LEFT JOIN a barras)
    // ==========================
    $sql = "
        SELECT
            LTRIM(RTRIM(items.f120_id)) AS item,
            LTRIM(RTRIM(i.f121_id_ext1_detalle)) AS ext1,
            LTRIM(RTRIM(i.f121_id_ext2_detalle)) AS ext2,
            LTRIM(RTRIM(bodega.f150_id)) AS bodega,
            LTRIM(RTRIM(bodega.f150_descripcion)) AS nombre,
            c.f415_cant_existencia_1 AS existencia,
            c.f415_costo_prom_tot    AS costo,
    ...
    // ==========================
    // 3) Reindexar: $mapExist[$key][$bodega]  donde key = item|ext1|ext2
    // ==========================
    $mapExist = [];
    foreach ($existencias as $ex) {
        ...
        // Por si vienen varias filas mismo key+bodega, acumulamos.
        $mapExist[$key][$bodega]['existencia'] += $existencia;
        $mapExist[$key][$bodega]['costo']      += $costo;

        $exisTotal = $mapExist[$key][$bodega]['existencia'];
        $costoTotal = $mapExist[$key][$bodega]['costo'];
        $mapExist[$key][$bodega]['precio_unitario'] = $exisTotal > 0 ? ($costoTotal / $exisTotal) : 0;
    }
    ...
    foreach ($rowsPreview as $r) {
        ...
        // 1) Bodega original
        $cantBd = $mapExist[$key][$bodega]['existencia'] ?? 0;
        $puBd   = $mapExist[$key][$bodega]['precio_unitario'] ?? 0;
        ...
        if ($restante > 0) {
            ...
            foreach ($otras as $bodId => $info) {
                if ($restante <= 0) break;
                if (($info['existencia'] ?? 0) <= 0) continue;
                ...
                $lineasGeneradas[] = [
                    ...
                    'costo_preview'   => $usar * (float)$info['precio_unitario'],
        ...
        if ($restante > 0) {
            ...
            // Dejar la línea de faltante para que se vea (con costo 0)
            $lineasGeneradas[] = [
                'item'            => $item,
                'ext1'            => $ext1,
                'ext2'            => $ext2,
        ...
        foreach ($lineasGeneradas as $ln) {
            if (($ln['cant_preview'] ?? 0) <= 0) continue;

            $esSinExist = !empty($ln['sin_existencia']);

            $precioUnit = $esSinExist
                ? 0
                : ((float)$ln['precio_unitario'] ?: (float)($r['precio_unitario'] ?? 0));
    ...
    // 5) Totales (solo de lo asignado; las líneas SIN EXISTENCIA suman costo 0, pero sí suman cantidad si tú las dejas)
    // Si NO quieres que "SIN EXISTENCIA" sume unidades, te lo ajusto (te lo dejo abajo).
