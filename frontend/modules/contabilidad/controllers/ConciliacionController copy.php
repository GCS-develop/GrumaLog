...
public function actionConciliacion($id = null)
{
    ...
    if ($id) {
        ...
        foreach ($doc->items as $it) {
            $rowsPreview[] = [
                ...
                'Ext1'            => trim((string)$it->extension1),
                'Ext2'            => trim((string)$it->extension2),
                'CodigoBarras'    => trim((string)($it->codigo_barras ?? '')),
                'BodegaId'        => $this->normalizeBodegaId($it->bodega),
                'cantidad_base'   => (int)$it->cantidad,
                'precio_unitario' => (float)$it->precio_unitario,
                'costo_total'     => (float)$it->costo_total,
        ...
        $meta = [
            ...
            'nit'           => $doc->proveedor_nit ?? '',   // ✅ IMPORTANTE
            'rowidTercero'  => null,                        // opcional: para resolver luego
            'fini'          => $doc->fecha_inicio,
            'ffin'          => $doc->fecha_fin,
            'totU'          => (int)$doc->total_unidades,
            'totC'          => (float)$doc->total_costo,
        ...
        foreach ($rowsPreview as &$r) {
            ...
            $r['CodigoBarras'] = trim((string)($r['CodigoBarras'] ?? ''));
            $r['Item']         = trim((string)($r['Item'] ?? ''));
            $r['Ext1']         = trim((string)($r['Ext1'] ?? ''));
            $r['Ext2']         = trim((string)($r['Ext2'] ?? ''));
        }
        unset($r);
    }
    ...
    // ==========================
    // 1) Armar lista de ITEMS únicos (NO depender de barras)
    // ==========================
    $itemsList = array_filter(array_map('trim', array_column($rowsPreview, 'Item')));
    $itemsList = array_values(array_unique($itemsList));

    if (empty($itemsList)) {
        Yii::$app->session->setFlash('error', 'El preview no contiene items válidos.');
        return $this->redirect(['index']);
    }

    $placeI = [];
    $params = [];
    foreach ($itemsList as $k => $it) {
        $ph = ":it$k";
        $placeI[]    = $ph;
        $params[$ph] = $it;
    }
    $itemsClause = implode(',', $placeI);

    // ==========================
    // 2) Traer existencias y COSTO TOTAL (F415_COSTO_PROM_TOT)
    //    Precio unitario = (SUM(costo_total) / SUM(existencia))
    // ==========================
    $sql = "
        SELECT
            ...
            LTRIM(RTRIM(bodega.f150_descripcion)) AS nombre,
            CAST(c.f415_cant_existencia_1 AS INT) AS existencia,
            CAST(c.F415_COSTO_PROM_TOT AS DECIMAL(18,6)) AS costo_total,
            LTRIM(RTRIM(barras.f131_id)) AS codbar
        ...
        LEFT JOIN t131_mc_items_barras barras
            ON barras.f131_rowid_item_ext = i.f121_rowid
           AND barras.f131_id_cia = 7
        WHERE c.f415_id_cia = 7
          AND items.f120_id IN ($itemsClause)
          AND c.f415_cant_existencia_1 > 0
        ORDER BY items.f120_id, c.f415_cant_existencia_1 DESC
    ";

    $existencias = Yii::$app->dbSiesa->createCommand($sql, $params)->queryAll();

    // ==========================
    // 3) Reindexar: $mapExist[$key][$bodega]
    //    key = item|ext1|ext2
    //    Acumula existencia y costo_total; calcula PU = costo_total / existencia
    // ==========================
    $mapExist = [];
    foreach ($existencias as $ex) {
        $item = trim((string)($ex['item'] ?? ''));
        $ext1 = trim((string)($ex['ext1'] ?? ''));
        $ext2 = trim((string)($ex['ext2'] ?? ''));
        $key  = $item . '|' . $ext1 . '|' . $ext2;

        $bodega        = $this->normalizeBodegaId($ex['bodega'] ?? '');
        $existencia    = (int)($ex['existencia'] ?? 0);
        $costoTotalFila = (float)($ex['costo_total'] ?? 0);

        if ($existencia <= 0) {
            continue;
        }

        if (!isset($mapExist[$key][$bodega])) {
            $mapExist[$key][$bodega] = [
                'existencia'      => 0,
                'costo_total'     => 0.0,
                'precio_unitario' => 0.0,
        ...
        // acumular por si hay varias filas del mismo item/ext/bodega
        $mapExist[$key][$bodega]['existencia']  += $existencia;
        $mapExist[$key][$bodega]['costo_total'] += $costoTotalFila;

        $exisTotal  = $mapExist[$key][$bodega]['existencia'];
        $costoTotal = $mapExist[$key][$bodega]['costo_total'];

        $mapExist[$key][$bodega]['precio_unitario'] = ($exisTotal > 0)
            ? ($costoTotal / $exisTotal)
            : 0.0;
    }

    // ==========================
    // 4) Procesar conciliación: usa key item|ext1|ext2
    // ==========================
    $conciliacion        = [];
    $rowsPreviewFiltrado = [];
    $descartados         = 0;
    ...
    foreach ($rowsPreview as $r) {
        ...
        $ext1         = trim((string)($r['Ext1'] ?? ''));
        $ext2         = trim((string)($r['Ext2'] ?? ''));
        ...
        if ($item === '' || $bodega === '') {
            $eliminados[] = [
                ...
                'Bodega'       => $bodega,
                'Motivo'       => 'Faltan datos clave (item/bodega)',
            ];
            $descartados++;
            continue;
        }
        ...
        // 1) Bodega original
        $cantBd = (int)($mapExist[$key][$bodega]['existencia'] ?? 0);
        $puBd   = (float)($mapExist[$key][$bodega]['precio_unitario'] ?? 0);
        ...
        if ($cantBd > 0) {
            ...
            $mapExist[$key][$bodega]['existencia'] -= $usar;
            $restante -= $usar;
        }

        // 2) Otras bodegas (ordenadas por mayor existencia)
        if ($restante > 0) {
            $otras = $mapExist[$key] ?? [];
            unset($otras[$bodega]);

            uasort($otras, fn($a, $b) => ((int)($b['existencia'] ?? 0)) <=> ((int)($a['existencia'] ?? 0)));

            foreach ($otras as $bodId => $info) {
                if ($restante <= 0) break;
                if ((int)($info['existencia'] ?? 0) <= 0) continue;

                $usar = min((int)$info['existencia'], $restante);
                ...
                $lineasGeneradas[] = [
                    ...
                    'bodega_final'    => $bodId,
                    'cant_preview'    => $usar,
                    'precio_unitario' => (float)($info['precio_unitario'] ?? 0),
                    'costo_preview'   => $usar * (float)($info['precio_unitario'] ?? 0),
                    'reemplazo'       => $info['nombre'] ?? '',
                    'sin_existencia'  => 0,
                ];

                $mapExist[$key][$bodId]['existencia'] -= $usar;
                $restante -= $usar;
            }
        }

        // 3) Si definitivamente no hay: registrar faltante (SIN EXISTENCIA)
        if ($restante > 0) {
            $eliminados[] = [
                'Item'         => $item,
                'CodigoBarras' => $codigoBarras,
                'Bodega'       => $bodega,
                'Motivo'       => "Sin existencia suficiente en ninguna bodega. Faltan $restante unidades.",
                'Faltante'     => $restante,
            ];
            ...
            $lineasGeneradas[] = [
                ...
                'bodega_original' => $bodega,
                'bodega_final'    => 'SIN',
                'cant_preview'    => $restante,
                'precio_unitario' => 0,
                'costo_preview'   => 0,
                'reemplazo'       => 'SIN EXISTENCIA',
                'sin_existencia'  => 1,
            ];
        }

        // 4) Pasar líneas generadas al preview
        foreach ($lineasGeneradas as $ln) {
            if ((int)($ln['cant_preview'] ?? 0) <= 0) continue;

            $esSinExist = !empty($ln['sin_existencia']);

            $precioUnit = $esSinExist
                ? 0
                : ((float)($ln['precio_unitario'] ?? 0) ?: (float)($r['precio_unitario'] ?? 0));
            ...
            $rowsPreviewFiltrado[] = [
                ...
                'cantidad_base'   => (int)$ln['cant_preview'],
                'precio_unitario' => $precioUnit,
                'costo_total'     => (int)$ln['cant_preview'] * $precioUnit,
                'sin_existencia'  => $esSinExist ? 1 : 0,
            ];
        }
    }
...
public function actionIndexLog()
{
    ...
    // ✅ Modelo de filtros (search)
    $search = new \yii\base\DynamicModel([
        'proveedor_id', 'proveedor_nit', 'estado', 'desde', 'hasta'
    ]);
    $search->addRule(['proveedor_id','proveedor_nit','estado'], 'string')
           ->addRule(['desde','hasta'], 'safe');
    ...
    // ✅ Aplicar filtros
    if (!empty($search->proveedor_id)) {
        $query->andWhere(['like', 'proveedor_id', trim($search->proveedor_id)]);
    }
    if (!empty($search->proveedor_nit)) {
        $query->andWhere(['like', 'proveedor_nit', trim($search->proveedor_nit)]);
    }
    if (!empty($search->estado)) {
        $query->andWhere(['estado' => $search->estado]);
    }
    if (!empty($search->desde)) {
        $query->andWhere(['>=', 'fecha_inicio', $search->desde]);
    }
    if (!empty($search->hasta)) {
        $query->andWhere(['<=', 'fecha_fin', $search->hasta]);
    }

    $query->orderBy(['fecha_realizado' => SORT_DESC]);
    ...
    $dataProvider = new ActiveDataProvider([
        ...
        'sort' => [
            'attributes' => [
                'id','proveedor_id','proveedor_nit','fecha_inicio','fecha_fin',
                'total_items_unicos','total_unidades','total_costo','estado','fecha_realizado'
            ],
            'defaultOrder' => ['fecha_realizado' => SORT_DESC],
        ]
    ]);
...
/**
 * Obtiene rowid del tercero (t200) a partir del NIT (t200_mm_terceros.f200_nit)
 */
private function getRowidTerceroByNit(string $nit): ?int
{
    $nit = trim($nit);
    if ($nit === '') return null;

    $rowid = Yii::$app->dbSiesa->createCommand("
        SELECT TOP 1 f200_rowid
        FROM t200_mm_terceros
        WHERE LTRIM(RTRIM(f200_nit)) = :nit
    ", [':nit' => $nit])->queryScalar();
...
/**
 * Determina si una bodega es inválida para envío a Siesa
 */
private function isInvalidBodegaForSend(string $bodega): bool
{
    $b = strtoupper(trim($bodega));
    return ($b === '' || $b === 'SIN' || $b === '000');
}
