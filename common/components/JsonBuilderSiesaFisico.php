...
class JsonBuilderSiesaFisico
{
     ...
     * @return string JSON
     */
    public static function buildOld(int $documentoId, int $conectorId = 4, string $rootKey = 'Físico'): string
    {
        if ($documentoId <= 0) {
    ...
    public static function build(int $documentoId, int $conectorId = 4, string $rootKey = 'Físico'): string
    {
        ...
        // 2) Campos configurados del conector
        $campos = SiesaConectorMovimientoCampo::find()
            ->select(['id', 'nombre_campo'])
            ->where(['conector_id' => $conectorId])
            ->all();
        ...
        // 4) VALORES CON CHUNKING (FIX 2100 PARAMS)
        $valores = [];

        foreach (array_chunk($movIds, 2000) as $chunk) {
            $partial = SiesaEnvioMovimientoValor::find()
                ->select(['movimiento_id', 'campo_id', 'valor'])
                ->where(['movimiento_id' => $chunk])
                ->orderBy(['movimiento_id' => SORT_ASC, 'campo_id' => SORT_ASC])
                ->asArray()
                ->all();

            if (!empty($partial)) {
                $valores = array_merge($valores, $partial);
            }
        }

        // 5) Reconstrucción estable por movimiento
        $rowsByMov = [];
        foreach ($movIds as $mid) {
            $rowsByMov[$mid] = [
                'Consecutivo' => '',
